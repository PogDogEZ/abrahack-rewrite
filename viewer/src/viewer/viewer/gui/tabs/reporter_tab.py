...
from ....pyclient.networking.handlers.handshake import LoginHandler
    ...
    class ConnectThread(QThread):
        ...
        def run(self) -> None:
            ...
            while self.main_window.connection.connected and not isinstance(self.main_window.connection.handler,
                                                                           DefaultHandler):
                if isinstance(self.main_window.connection.handler, LoginHandler):
                    if self.main_window.connection.handler.state == LoginHandler.State.HAND_SHAKE:
                        self.progress_emitter.emit(30)
                    elif self.main_window.connection.handler.state == LoginHandler.State.ENCRYPTION:
                        self.progress_emitter.emit(50)
                    elif self.main_window.connection.handler.state == LoginHandler.State.LOGIN:
