...
class DefaultHandler(Handler):
    """
    The handler for default stuff, duh.
    """
    ...
    def on_packet(self, packet: Packet) -> None:
        ...
        if isinstance(packet, DisconnectPacket):
            ...
            logging.debug("Connection info response:")
            logging.debug("Ping: %ims." % packet.ping)
            logging.debug("User: %r." % packet.user)
            logging.debug("IP address: %s:%i." % (packet.host, packet.port))
